...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    const joinAmenityValues = [...amenityValues]
    ...
    /** * UPDATE ***/
    let dataToUpdate = [...berlinData.filter(hasDock)]
    ...
    for (let i = 0; i < dataToUpdate.length; i++) {
      ...
      if (data.dock) {
        data.seatAmenityId = seatAmenitiesToUpdateIdx[`${data.seatId}-${dockAmenityId}`]
      }
    }
    ...
    for (const data of dataToUpdate) {
      await qsq(
        `UPDATE "seatAmenityMultiValues" SET "amenityValueId" = ${data.dockAmenityValueId} WHERE "seatAmenityId" = ${data.seatAmenityId}`
      )
    }
    ...
    const seatAmenities = await queryInterface.bulkInsert(
      'seatAmenities',
      dataToAdd.filter(hasDock).map((data) => {
