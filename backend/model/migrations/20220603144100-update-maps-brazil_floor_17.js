...
const brasilDataSeventeenFloor = require('./data/20220602163000-update-seats-brazil-floor-17.json')
...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    let seatOwnersOfFloor = await queryInterface.sequelize.query(`
          ...
          WHERE f.number = 17 and bu.name = 'Rochavera' and si.name = 'Morumbi'`)
    ...
    const areaA2Id = (
      await queryInterface.sequelize.query(`
          SELECT a.id FROM areas a
             inner join floors f on f.id = a."floorId"
             inner join buildings b on b.id = f."buildingId"
          WHERE name = 'Rochavera' and f.number = 17 and a.code = 'a2'`)
    )[0][0].id
    ...
    const floorArea = await fs.readFile('./model/migrations/maps/br/rochavera/17/areas.svg', 'utf8')
    const areaMapA2 = await fs.readFile('./model/migrations/maps/br/rochavera/17/a2.svg', 'utf8')
    const areaMapB1 = await fs.readFile('./model/migrations/maps/br/rochavera/17/b1.svg', 'utf8')
    const areaMapB2 = await fs.readFile('./model/migrations/maps/br/rochavera/17/b2.svg', 'utf8')
    ...
    // INSERT NEW SEATS
    for (let i = 0; i < brasilDataSeventeenFloor.length; i++) {
      switch (brasilDataSeventeenFloor[i].area) {
        case 'a2':
          brasilDataSeventeenFloor[i].areaId = areaA2Id
          break
        case 'b1':
          brasilDataSeventeenFloor[i].areaId = areaB1Id
          break
        case 'b2':
          brasilDataSeventeenFloor[i].areaId = areaB2Id
          break
      }
    }
    ...
    await queryInterface.bulkInsert(
      'seats',
      brasilDataSeventeenFloor.map(({ seat, areaId, available }) => {
