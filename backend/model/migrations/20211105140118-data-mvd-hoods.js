const hoodsData = require('./data/20211105140118-mvdo-hoods.json')
...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    /** *  Delete ***/
    await queryInterface.sequelize.query(`DELETE FROM "hoodOrgCodes";`, { transaction })
    await queryInterface.sequelize.query(`DELETE FROM "seatHoods";`, { transaction })
    await queryInterface.sequelize.query(`DELETE FROM hoods;`, { transaction })
    ...
    Object.entries(seatHoodsData).map(([h, v]) => {
      return Object.entries(v).map(([f, seats]) => {
        return seats.forEach((seat) => {
          valuesSeat += `('HUB Montevideo','WTC Torre 4',${f},'-',${seat}),`
        })
      })
    })
