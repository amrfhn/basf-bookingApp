...
const brasilDataTwelveFloor = require('./data/20220602110700-update-seats-brazil-floor-12.json')
...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    let seatOwnersOfFloor = await queryInterface.sequelize.query(`
          ...
          WHERE f.number = 12 and bu.name = 'Rochavera' and si.name = 'Morumbi'`)
    ...
    let areasC1AndC2Ids = await queryInterface.sequelize.query(`
        ...
        WHERE name = 'Rochavera' and f.number = 12 and (a.code = 'c1' or a.code = 'c2')`)
    ...
    // QUERIES TO UPDATE MAPS IN FLOOR 12
    const floorAreaId = (
    ...
    // INSERT NEW SEATS
    for (let i = 0; i < brasilDataTwelveFloor.length; i++) {
      switch (brasilDataTwelveFloor[i].area) {
        case 'a1':
          brasilDataTwelveFloor[i].areaId = areaA1Id
          break
        case 'a2':
          brasilDataTwelveFloor[i].areaId = areaA2Id
          break
        case 'b1':
          brasilDataTwelveFloor[i].areaId = areaB1Id
          break
        case 'b2':
          brasilDataTwelveFloor[i].areaId = areaB2Id
          break
      }
    }
    ...
    await queryInterface.bulkInsert(
      'seats',
      brasilDataTwelveFloor.map(({ seat, areaId, available }) => {
